...
void OLED_ShowVelocity(float velocity)
{
	int ge,shi,bai,qian;
	int vel;
	OLED_ShowStr(0,5,"V:",1);
	OLED_ShowStr(56,5,"m/s",1);
	...
	if(vel>0)
	{
		...
		OLED_ShowSNum(16,5,qian,1);
		OLED_ShowStr(24,5,".",1);
		OLED_ShowSNum(32,5,bai,1);
		OLED_ShowSNum(40,5,shi,1);
		OLED_ShowSNum(48,5,ge,1);
	}
	else
	{
  	OLED_ShowStr(10,5,"-",1);
...
void OLED_ShowPWM(int PWM)
{
	...
	if(PWM<0)
	{
		OLED_ShowStr(90,7,"-",1);		//显示负号
		PWM=-PWM;	
	}
	else OLED_ShowStr(90,7," ",1); //掩盖负号
	OLED_Shownum1(96,7,PWM,1);	    
...
void OLED_ShowAngle(float roll,float yaw)
{
	...
	if(Roll<0)
	{
		OLED_ShowStr(32,3,"-",1);		//显示负号
		Roll=-Roll;	
	}
	else OLED_ShowStr(32,3," ",1); //掩盖负号
	OLED_Shownum3(40,3,Roll,1);	  //显示翻滚角
	...
	else OLED_ShowStr(24,4," ",1); 
	OLED_Shownum3(32,4,Yaw,1);	    //显示航向角
}
void OLED_ShowTrace(uint8_t* value)
{
	OLED_ShowStr(0,0,"Trace:",1);
	OLED_ShowSNum(48,0,value[0],1);
	OLED_ShowSNum(56,0,value[1],1);
	OLED_ShowSNum(64,0,value[2],1);
	OLED_ShowSNum(72,0,value[3],1);
	OLED_ShowSNum(80,0,value[4],1);
...
//功能：在一行中显示五位数字
//wgn 2021.4.29
void OLED_ShowNum(unsigned char x, unsigned char y,int num, unsigned char TextSize)
{
