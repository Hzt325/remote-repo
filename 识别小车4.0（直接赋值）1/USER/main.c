...
//				}
		...
		{	
			if(Turn_CD_flag==0)
			{
				if((Read_Xunji(xunji_1)==0)&&(turn_left_flag==0)&&(Turn_Stop_Flag==0))//左直角转
				{		
					Turn_Stop_Flag=1;
					turn_left_flag=1;
				}
				...
				if((Read_Xunji(xunji_5)==0)&&(turn_right_flag==0)&&(Turn_Stop_Flag==0))//右直角转
				{							
					Turn_Stop_Flag=1;
					turn_right_flag=1;				
				}	
...
void Oled_disp()
{
	...
	OLED_ShowSNum(92,2,Turn_count,1);
	
	OLED_ShowTrace(xunji_vaule);
	OLED_ShowPWM(pwm_left);
	OLED_ShowSNum(72,1,Turn_Stop_Flag,1);
	OLED_ShowSNum(72,2,turn_left_flag,1);
